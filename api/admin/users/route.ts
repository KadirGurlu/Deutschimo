...
export async function GET(request: Request) {
  ...
  const [users, total] = await prisma.$transaction([
    prisma.user.findMany({
      ...
      take: pageSize,
      select: { id: true, firstName: true, lastName: true, email: true, role: true, status: true, currentLevel: true, targetLevel: true, createdAt: true, lastSeenAt: true, emailVerified: true },
    }),
