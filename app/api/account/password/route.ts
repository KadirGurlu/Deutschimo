...
async function POSTHandler(request: Request) {
  ...
  if (await verifyPassword(newPassword, user.passwordHash)) return NextResponse.json({ error: "Yeni şifre mevcut şifreden farklı olmalıdır." }, { status: 400 });

  await prisma.user.update({ where: { id: current.id }, data: { passwordHash: await hashPassword(newPassword) } });
  await Promise.all([
    prisma.session.deleteMany({ where: { userId: current.id } }),
    revokeUserSessions(current.id),
    writeAuditLog({ actorUserId: current.id, actorEmail: current.email, action: "ACCOUNT_PASSWORD_CHANGED", entityType: "User", entityId: current.id, summary: "Kullanıcı şifresini değiştirdi ve mevcut oturumları iptal etti." }),
  ]);
