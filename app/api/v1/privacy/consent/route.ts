...
export async function PUT(request: Request) {
  ...
  if (!body || typeof body !== "object" || Array.isArray(body)) {
    return apiError("INVALID_BODY", "Invalid consent payload.", { status: 422 });
  }
  const value = body as Record<string, unknown>;
  if (typeof value.analyticsEnabled !== "boolean") {
    return apiError("INVALID_ANALYTICS_CONSENT", "analyticsEnabled must be boolean.", { status: 422 });
  }
  ...
  const consent = await prisma.userConsent.upsert({
    ...
    update: {
      ...
      functionalEnabled: typeof value.functionalEnabled === "boolean" ? value.functionalEnabled : true,
      marketingEnabled: typeof value.marketingEnabled === "boolean" ? value.marketingEnabled : false,
      policyVersion:
