...
export default function DashboardPage() {
  ...
  return <div className="dashboard-shell"><AppSidebar active="dashboard"/><section className="dashboard-main">
    <div className="welcome"><div><span className="eyebrow">ÖĞRENCİ PANELİ</span><h1>Tekrar hoş geldin, Kadir.</h1><p>Bugünkü 30 dakikalık çalışma hedefini sürdür.</p></div><Link className="button button-primary" href={continueUnit ? `/learn/${a1.id}/${continueUnit.id}` : `/courses/${a1.slug}`}>Derse Devam Et</Link></div>
    <div className="stats-grid"><Stat icon={<Target/>} label="Günlük hedef" value="12 / 30 dk" note="18 dakika kaldı"/><Stat icon={<Flame/>} label="Çalışma serisi" value="6 gün" note="Kişisel rekor: 11 gün"/><Stat icon={<BookOpenCheck/>} label="Tamamlanan ünite" value={String(store.completedCount)} note={`${store.inProgressCount} ünite devam ediyor`}/><Stat icon={<TrendingUp/>} label="Ortalama quiz" value={`%${averageQuiz}`} note={`${quizScores.length} değerlendirme`}/></div>
