...
function toLevel(value: unknown, fallback: Level): Level {
  return typeof value === "string" &&
    Object.values(Level).includes(value as Level)
    ? (value as Level)
    : fallback;
}
...
const adapter = PrismaAdapter(prisma) as NonNullable<
  ...
  callbacks: {
    ...
    async jwt({
      ...
      if (user) {
        ...
        token.userRefreshedAt =
          Date.now();

        token.authenticatedAt =
          Date.now();

        token.revocationCheckedAt =
          Date.now();
        ...
        Boolean(
          userId &&
            (
              ...
              Date.now() -
                lastRefresh >=
                TRUSTED_USER_REFRESH_MS
            ),
        );

      if (shouldRefresh) {
        const databaseUser =
          await trustedUser(userId);
      ...
      // Never copy client-provided session.update()
      // fields into the signed token.
      return token;
    },
    ...
    async session({
      ...
      if (session.user) {
        ...
        session.user.firstName =
          typeof token.firstName ===
          "string"
            ? token.firstName
            : null;

        session.user.lastName =
          typeof token.lastName ===
          "string"
            ? token.lastName
            : null;
