...
type AdminUser = {
  ...
  createdAt: string; lastSeenAt: string | null; emailVerified: string | null; isTestUser:boolean;
};
...
export function UserManager() {
  const [users,setUsers]=useState<AdminUser[]>([]); const [query,setQuery]=useState(""); const [testFilter,setTestFilter]=useState("all"); const [total,setTotal]=useState(0); const [loading,setLoading]=useState(true); const [error,setError]=useState("");
  const load=useCallback(async()=>{setLoading(true);setError("");try{const response=await fetch(`/api/admin/users?q=${encodeURIComponent(query)}${testFilter==="all"?"":`&test=${testFilter}`}`,{cache:"no-store"});const payload=await response.json() as {users?:AdminUser[];total?:number;error?:string};if(!response.ok)throw new Error(payload.error);setUsers(payload.users??[]);setTotal(payload.total??0);}catch(caught){setError(caught instanceof Error?caught.message:"Kullanıcılar yüklenemedi.");}finally{setLoading(false);}},[query,testFilter]);
  useEffect(()=>{const timer=setTimeout(load,250);return()=>clearTimeout(timer)},[load]);

  async function update(id:string,changes:Partial<Pick<AdminUser,"role"|"status"|"currentLevel"|"targetLevel"|"isTestUser">>){const response=await fetch(`/api/admin/users/${id}`,{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify(changes)});const payload=await response.json() as {user?:AdminUser;error?:string};if(!response.ok){setError(payload.error??"Güncelleme başarısız.");return;}if(payload.user)setUsers((current)=>current.map((user)=>user.id===id?payload.user!:user));}
