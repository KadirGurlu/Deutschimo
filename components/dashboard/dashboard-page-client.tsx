...
export function DashboardPageClient() {
  ...
  const streak = calculateStudyStreak([...store.state.activities.map((activity) => activity.createdAt), ...store.state.studySessions.map((studySession) => studySession.startedAt), ...Object.values(store.state.learningPositions).map((learningPosition) => learningPosition.updatedAt)]);
  const continueHref = continueUnit ? `/learn/${course.id}/${continueUnit.id}` : `/courses/${course.slug}`;
  return <div className="dashboard-shell"><AppSidebar active="dashboard"/><section className="dashboard-main">
    ...
    <div className="stats-grid"><Stat icon={<BookOpenCheck/>} label="Devam edilen kurs" value={course.level} note={course.title}/><Stat icon={<Flame/>} label="Çalışma serisi" value={`${streak.current} gün`} note={streak.best ? `Kişisel rekor: ${streak.best} gün` : "İlk çalışmanla serini başlat"}/><Stat icon={<TrendingUp/>} label="Kurs ilerlemesi" value={`%${coursePercent}`} note={`${completedCount}/${courseUnits.length} ünite tamamlandı`}/><Stat icon={<Target/>} label="Günlük hedef" value={`${dailyGoal} dk`} note="Kişisel planına göre dağıtılır"/></div>
    ...
    <IntelligenceOverviewPanel/><SkillDashboardPanel/><VocabularyDashboardCard/>
