...
import { answersMatch, normalizeAnswer } from "@/lib/learning/answer-normalizer";
...
export function ExerciseShell({ course, unit }: { course: Course; unit: Unit }) {
  ...
  const firstUnfinished = Math.max(0, exercises.findIndex((exercise) => !progress.state.exerciseAttempts.some((attempt) => attempt.exerciseId === exercise.id)));
  const [index, setIndex] = useState(firstUnfinished < 0 ? exercises.length - 1 : firstUnfinished);
  ...
  const percent = Math.round(((completedCount + (alreadySubmitted ? 0 : 1)) / exercises.length) * 100);
  ...
  return <main className="exercise-page-shell">
    ...
    <article className="exercise-step-card">
      ...
      <ExerciseRenderer exercise={exercise} value={answer} onChange={(value) => { setAnswer(value); setChecked(false); }} disabled={accepted}/>
      {checked ? <ExerciseFeedback correct={correct || alreadySubmitted} explanation={exercise.explanation} relatedSlideHref={`/learn/${course.id}/${unit.id}`} canRetry={!accepted}/> : null}
