...
export function UnitQuiz({ course, unit, nextUnitId }: { course: Course; unit: Unit; nextUnitId?: string }) {
  ...
  if (submitted && result) return <div className="quiz-page-wrap"><QuizResult {...result} courseId={course.id} unitId={unit.id} nextUnitId={nextUnitId} onRetry={() => { setSubmitted(false); setResult(undefined); setConfirming(false); setIndex(0); setAnswers({}); }}/></div>;
  ...
  return <main className="quiz-page-wrap">
    ...
    <article className="quiz-question-card"><span className="level-badge">{question.topic}</span><h2>{question.prompt}</h2>
      {question.type === "MULTIPLE_CHOICE" ? <fieldset className="choice-grid"><legend className="sr-only">Cevap seç</legend>{question.options?.map((option) => <label key={option.id} className={currentAnswer === option.value ? "selected" : ""}><input type="radio" name={question.id} checked={currentAnswer === option.value} onChange={() => setAnswers((current) => ({ ...current, [question.id]: option.value }))}/><span>{option.label}</span></label>)}</fieldset> : question.type === "TRUE_FALSE" ? <div className="true-false-grid"><button type="button" className={currentAnswer === true ? "selected" : ""} onClick={() => setAnswers((current) => ({ ...current, [question.id]: true }))}>Doğru</button><button type="button" className={currentAnswer === false ? "selected" : ""} onClick={() => setAnswers((current) => ({ ...current, [question.id]: false }))}>Yanlış</button></div> : <label className="exercise-text-field"><span>Cevabın</span><input value={String(currentAnswer ?? "")} onChange={(event) => setAnswers((current) => ({ ...current, [question.id]: event.target.value }))}/></label>}
