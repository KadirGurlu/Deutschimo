...
export function LessonScreen({ course, unit }: { course: Course; unit: Unit }) {
  ...
  const canNext = Boolean(slide && miniAnswered && minTimePassed);
  ...
  return <div className="new-learning-shell">
    ...
    <main className="slide-workspace">
      ...
      <LessonSlide slide={slide} miniAnswer={miniAnswers[slide.id]} onMiniAnswer={(value) => setMiniAnswers((current) => ({ ...current, [slide.id]: value }))}/>
      <SlideNavigation canPrevious={index > 0} canNext={canNext && (!isLast || allOtherRequiredDone)} isLast={isLast} onPrevious={() => selectSlide(index - 1)} onNext={completeAndMove} nextHint={!miniAnswered ? "Devam etmek için mini kontrolü cevapla." : !minTimePassed ? "İçeriği incelemek için birkaç saniye ayır." : isLast && !allOtherRequiredDone ? "Alıştırmalara geçmek için zorunlu slaytları tamamla." : undefined}/>
    </main>
    <aside className="lesson-right-panel">
      ...
      <section><NotebookPen size={20}/><h3>Ders notlarım</h3><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Notlarını yaz..."/><button className="button button-secondary" onClick={() => { window.localStorage.setItem(`deutschimo-note-${unit.id}`, notes); setSaveStatus("saved"); }}>Notu Kaydet</button></section>
    </aside>
  </div>;
}
