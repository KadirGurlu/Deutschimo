  ...
  return <>
    ...
    <aside className={`unit-sidebar ${open ? "open" : ""}`}>
      ...
      <nav aria-label="Kurs üniteleri">{units.map((unit) => {
        ...
        return <Link aria-disabled={status === "LOCKED"} onClick={(event: { preventDefault: () => void }) => { if (status === "LOCKED") event.preventDefault(); else setOpen(false); }} className={`unit-sidebar-link ${unit.id === currentUnitId ? "active" : ""} ${status === "LOCKED" ? "locked" : ""}`} href={`/learn/${course.id}/${unit.id}`} key={unit.id}>
