...
export function ReadingLab() {
  ...
  async function saveWord(item:VocabularyItem){const response=await fetch("/api/skills/vocabulary",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({...item,sourceSkill:"READING",sourceTaskId:task.id,sourceCourseId:task.level.toLowerCase(),sourceUnitTitle:task.title})});if(response.ok)setSavedWords((current)=>[...new Set([...current,item.word])]);}
