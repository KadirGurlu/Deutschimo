...
function exerciseTypes(courseId) {
...
function createFillFromExample(content, exampleIndex, targetOffset = 1) {
...
function createExercise(unit, order, type) {
    const content = getCurriculumContent(unit.id);
    const id = `${unit.id}-v7-e${order}`;
    const common = {
        id,
        unitId: unit.id,
        groupId: `${unit.id}-practice-v7`,
        order,
        type,
    ...
    switch (type) {
        case "MULTIPLE_CHOICE": {
            ...
            const choices = makeChoices(target.tr, vocabulary.filter((item) => item.de !== target.de).map((item) => item.tr), unitSeed(unit, order), "Alternatif anlam");
            return {
                ...common,
...
export const exercises = units.flatMap((unit) => exerciseTypes(unit.courseId).map((type, index) => createExercise(unit, index + 1, type)));
function grammarQuestion(unit, content) {
...
function quizGermanChoice(unit, content, questionOrder, exampleIndex) {
    const example = content.examples[exampleIndex % content.examples.length];
    const choices = makeChoices(example.de, content.examples.filter((_, index) => index !== exampleIndex % content.examples.length).map((item) => item.de), unitSeed(unit, questionOrder + 20), "Alternatif cümle");
    return {
        id: `${unit.id}-v7-q${questionOrder}`,
...
function quizVocabularyQuestion(unit, content) {
    const vocabulary = content.vocabulary.map(parseVocabulary);
    const targetIndex = (unit.order * 3 + 1) % vocabulary.length;
    const target = vocabulary[targetIndex];
    const choices = makeChoices(target.tr, vocabulary.filter((_, index) => index !== targetIndex).map((item) => item.tr), unitSeed(unit, 44), "Alternatif anlam");
    return {
        id: `${unit.id}-v7-q4`,
        type: "MULTIPLE_CHOICE",
...
function quizVocabularyFill(unit, content) {
    const vocabulary = content.vocabulary.map(parseVocabulary);
...
function quizGrammarFill(unit, content, questionOrder, rowOffset) {
    const personColumn = content.grammarColumns[0];
    const formColumn = content.grammarColumns[1] ?? content.grammarColumns[0];
...
function quizGrammarChoice(unit, content) {
    const personColumn = content.grammarColumns[0];
    const formColumn = content.grammarColumns[1] ?? content.grammarColumns[0];
    const row = (unit.order + 1) % Math.min(personColumn.values.length, formColumn.values.length);
    const person = personColumn.values[row];
    const answer = formColumn.values[row];
    const choices = makeChoices(answer, formColumn.values.filter((_, index) => index !== row), unitSeed(unit, 55), "Alternatif biçim");
    return {
        id: `${unit.id}-v7-q5`,
        type: "MULTIPLE_CHOICE",
...
export const quizzes = units.map((unit) => {
    const content = getCurriculumContent(unit.id);
    const meaningExample = content.examples[1 % content.examples.length];
    const meaningChoices = makeChoices(meaningExample.tr, content.examples.filter((_, index) => index !== 1 % content.examples.length).map((item) => item.tr), unitSeed(unit, 61), "Alternatif anlam");
    const questions = [
...
export const totalExerciseCounts = { A1: 120, A2: 160, B1: 180, B2: 200 };
