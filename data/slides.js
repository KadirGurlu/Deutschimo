...
const block = (id, type, extra = {}) => ({ id, type, ...extra });
...
const genericSeparablePrefixes = ["herunter", "zurück", "zusammen", "hervor", "entgegen", "weiter", "fertig", "statt", "hoch", "teil", "ab", "an", "auf", "aus", "ein", "fest", "mit", "nach", "vor", "weg", "zu"];
...
function getSeparableVerb(infinitive) {
    if (separableVerbs[infinitive])
        return separableVerbs[infinitive];
    if (genericSeparableExceptions.has(infinitive))
        return undefined;
...
function conjugateSimple(infinitive) {
    if (irregularPresent[infinitive])
        return irregularPresent[infinitive];
    const separable = getSeparableVerb(infinitive);
    if (separable)
        return conjugateSimple(separable.base).map((form) => `${form} ${separable.prefix}`);
    return regularForms(infinitive);
}
function getVerbInfo(term) {
    const cleaned = term.trim();
    if (!cleaned || nonVerbTerms.has(cleaned) || /^(der|die|das|ein|eine)\s/.test(cleaned))
        return undefined;
    ...
    if (!candidate || /^[A-ZÄÖÜ]/.test(candidate))
        return undefined;
...
function conjugationColumns(term) {
    ...
    const prepositionCases = { an: "an + Akk.", auf: "auf + Akk.", gegen: "gegen + Akk.", um: "um + Akk.", mit: "mit + Dat.", von: "von + Dat.", zu: "zu + Dat." };
...
function grammarDeepening(title, level) {
    const normalized = title.toLocaleLowerCase("de-DE");
    if (normalized.includes("passiv"))
        return [
    ...
    if (normalized.includes("konjunktiv"))
        return [
    ...
    if (normalized.includes("perfekt") || normalized.includes("präteritum") || normalized.includes("plusquamperfekt") || normalized.includes("vergangen"))
        return [
    ...
    if (normalized.includes("relativ") || normalized.includes("nebensatz") || normalized.includes("konnektor") || normalized.includes("satz"))
        return [
    ...
    if (normalized.includes("adjektiv"))
        return [
    ...
    if (normalized.includes("präposition") || normalized.includes("dativ") || normalized.includes("akkusativ"))
        return [
...
function createUnitSlides(unit) {
    ...
    const conjugations = vocabularyTerms
        ...
        .filter((item) => Boolean(item.columns));
    const exampleBlocks = content.examples.flatMap((example, index) => [
    ...
    const conjugationBlocks = conjugations.length ? conjugations.flatMap((item, index) => [
...
export const slides = units.flatMap((unit) => createUnitSlides(unit));
