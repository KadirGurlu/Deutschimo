...
import { buildCommonMistakes, buildDialogue, buildListeningLines, buildPrerequisites, buildReadingLines, buildRegisterExamples, buildRichVocabulary, buildStructureExamples, buildTopicCheckpoint, buildUseCases } from "@/lib/learning/content-enrichment";
...
function createUnitSlides(unit: Unit): LessonSlide[] {
  ...
  const readingLines = buildReadingLines(content, unit);
  const listeningLines = buildListeningLines(content, unit);
  ...
  const definitions: Array<Pick<LessonSlide, "title" | "completionRule" | "estimatedMinutes"> & { blocks: ContentBlock[] }> = [
    ...
    },
    {
      ...
      blocks: [
        ...
        block(`${unitId}-reading`, "reading_text", { title: "Okuma metni", lines: readingLines }),
    ...
    },
    {
      ...
      blocks: [
        ...
        block(`${unitId}-listening`, "listening_text", { title: "Dinleme metni ve Türkçe anlamı", lines: listeningLines }),
