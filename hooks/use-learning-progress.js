...
import { LEARNING_EVENT, readLearningState, resetLearningState, readContentState } from "@/lib/storage/learning-storage";
export function useLearningProgress(course) {
    const [state, setState] = useState(() => readLearningState());
    ...
    const unitProgressMap = useMemo(() => {
        ...
        for (const unit of mergedUnits) {
            ...
            if (quiz)
                map[unit.id] = calculateUnitProgress(state, unit, unitSlides, unitExercises, quiz);
        }
        return map;
    ...
    const getStatus = useCallback((unitId) => {
        const unit = mergedUnits.find((item) => item.id === unitId);
        if (!unit)
            return "NOT_STARTED";
        if (isUnitLocked(unit, unitProgressMap))
            return "LOCKED";
