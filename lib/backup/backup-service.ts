...
export async function createLogicalBackup() {
  ...
  try {
    ...
    const [users, accounts, enrollments, snapshots, progress, activities, studySessions, placement, insights, plans, reviews, skillAttempts, vocabularyNotebook, courses, units, exercises] = await Promise.all([
      ...
      prisma.learningInsightSnapshot.findMany(), prisma.dailyStudyPlan.findMany(), prisma.smartReviewState.findMany(), prisma.skillLabAttempt.findMany(), prisma.vocabularyNotebookItem.findMany(), prisma.course.findMany(), prisma.unit.findMany(), prisma.exercise.findMany(),
    ]);
    const payload = { schemaVersion: "13.0", createdAt: new Date().toISOString(), data: { users, accounts, enrollments, snapshots, progress, activities, studySessions, placement, insights, plans, reviews, skillAttempts, vocabularyNotebook, courses, units, exercises } };
