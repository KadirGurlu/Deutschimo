...
export function calculateUnitProgress(
  ...
  let stage: UnitProgress["stage"] = "LESSONS";
  if (lessonsDone) stage = "EXERCISES";
  if (lessonsDone && exercisesDone) stage = "QUIZ";
  if (canComplete) stage = "COMPLETED";
  ...
  const status: UnitProgress["status"] = canComplete ? "COMPLETED" : hasActivity ? "IN_PROGRESS" : "NOT_STARTED";
...
export function calculateCourseProgress(unitProgress: UnitProgress[]): number {
  if (!unitProgress.length) return 0;
