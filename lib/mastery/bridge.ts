...
function skillLabCorrect(...roots:unknown[]){const values:boolean[]=[];const q=[...roots],seen=new Set<unknown>();while(q.length){const c=q.shift();if(!c||typeof c!=="object"||seen.has(c))continue;seen.add(c);if(Array.isArray(c)){for(const x of c)q.push(x);continue;}const o=rec(c);const value=bool(o.correct,o.isCorrect);if(value!==null)values.push(value);for(const x of Object.values(o))if(x&&typeof x==="object")q.push(x);}return values.length?values.every(Boolean):null;}
...
export async function captureMasteryExchange(input:{source:string;requestBody:unknown;responseBody?:unknown}):Promise<void>{
  ...
  const multiCorrect=src==="SKILL_LAB"?skillLabCorrect(request,response):null;
  if(scores.length){await Promise.all(scores.map(({skill,score})=>recordMasteryEvidence(userId,{courseId,unitId,questionId:`${qid}:${skill.toLowerCase()}`,skill,tags:inferMasteryTags({skill,courseId,unitId,prompt:qid}),score,correct:multiCorrect,source:src,externalId:externalId?`${externalId}:${skill}`:null})));return;}
  const score=num(deep(response,["score","overallScore","percent","mastery"]),deep(request,["score","percent"]));
  const correct=bool(deep(response,["correct","isCorrect","passed"]),deep(request,["correct","isCorrect"]));
  const finalCorrect=src==="SKILL_LAB"?(multiCorrect??correct):correct;
  ...
  await recordMasteryEvidence(userId,{courseId,unitId,questionId:qid,skill,tags:inferMasteryTags({skill,courseId,unitId,prompt:deep(request,["prompt","question","title"]),section:deep(request,["section","kind","type"]),existing:deep(request,["masteryTags","tags"])??deep(response,["masteryTags","tags"])}),score:score??undefined,correct:finalCorrect,responseMs:num(deep(request,["responseMs","durationMs","elapsedMs"])),hintUsed:bool(deep(request,["hintUsed","usedHint"]))??false,confidence:str(deep(request,["confidence"]))??null,difficulty:num(deep(request,["difficulty"])),source:src,externalId});
}
