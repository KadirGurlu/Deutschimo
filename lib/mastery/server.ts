...
async function updateSkill(tx:Prisma.TransactionClient,userId:string,input:MasteryEvidenceInput,skill:MasterySkillArea,unitId:string|null,score:number,weight:number){
  const key=scopeKey(input.courseId,unitId), where={userId_scopeKey_skill:{userId,scopeKey:key,skill}};
  ...
  await tx.masterySkillSnapshot.upsert({where,create:{userId,scopeKey:key,courseId:input.courseId,unitId,skill,score:next.score,evidenceCount:next.evidenceCount,confidence:next.confidence,lastEvidenceAt:new Date()},update:{score:next.score,evidenceCount:next.evidenceCount,confidence:next.confidence,lastEvidenceAt:new Date()}});
}
async function updateTopic(tx:Prisma.TransactionClient,userId:string,input:MasteryEvidenceInput,tag:string,unitId:string|null,score:number,weight:number){
  const key=scopeKey(input.courseId,unitId), where={userId_scopeKey_tag:{userId,scopeKey:key,tag}};
  const prev=await tx.masteryTopicSnapshot.findUnique({where});
  const next=evolveSnapshot({previousScore:prev?.score??null,evidenceCount:prev?.evidenceCount??0,evidenceScore:score,evidenceWeight:weight,correct:input.correct});
  await tx.masteryTopicSnapshot.upsert({where,create:{userId,scopeKey:key,courseId:input.courseId,unitId,tag,score:next.score,evidenceCount:next.evidenceCount,confidence:next.confidence,lastEvidenceAt:new Date()},update:{score:next.score,evidenceCount:next.evidenceCount,confidence:next.confidence,lastEvidenceAt:new Date()}});
}
export async function recordMasteryEvidence(userId:string,raw:MasteryEvidenceInput):Promise<void>{
  const courseId=raw.courseId.trim().toLowerCase(); if(!courseId||!raw.questionId.trim()) return;
  ...
  const input={...raw,courseId,skill,tags}; const {evidenceScore,evidenceWeight}=normalizeEvidence(input); const key=eventKey(userId,input);
  try{
    await prisma.$transaction(async tx=>{
      await tx.masteryAttempt.create({data:{userId,courseId,unitId:input.unitId??null,questionId:input.questionId.slice(0,180),source:input.source,skill:skill as MasterySkillArea,tags,score:evidenceScore,correct:input.correct??null,responseMs:input.responseMs??null,hintUsed:Boolean(input.hintUsed),confidenceLabel:input.confidence?.slice(0,40)??null,difficulty:input.difficulty??null,evidenceWeight,eventKey:key}});
...
async function completionByCourse(userId:string){
  ...
  try{
    const rows=await prisma.$queryRawUnsafe<Array<{courseId:string;completion:number|string|null}>>(`SELECT LOWER(COALESCE(c."slug", c."level"::text, u."courseId")) AS "courseId", AVG(COALESCE(up."totalProgress",0)) AS "completion" FROM "UnitProgress" up JOIN "Unit" u ON u."id"=up."unitId" LEFT JOIN "Course" c ON c."id"=u."courseId" WHERE up."userId"=$1 GROUP BY LOWER(COALESCE(c."slug", c."level"::text, u."courseId"))`,userId);
    for(const row of rows){const n=Number(row.completion); if(row.courseId&&Number.isFinite(n)) result.set(String(row.courseId).toLowerCase(),n);}
  }catch{}
  return result;
}
function summarize(rows:Array<{skill:MasterySkillArea;score:number;evidenceCount:number;confidence:number}>):MasterySkillSummary[]{
  ...
  return MASTERY_SKILLS.map(skill=>{const x=map.get(skill);return {skill,score:x?Math.round(x.score):null,evidenceCount:x?.evidenceCount??0,confidence:x?.confidence??0};});
}
function aggregate(skills:MasterySkillSummary[]){
  const measured=skills.filter(x=>x.score!==null),coverage=Math.round(measured.length/6*100);
  if(!measured.length) return {mastery:null,coverage,provisional:true,strongestSkill:null,weakestSkill:null};
  const w=measured.reduce((a,x)=>{const k=Math.max(.25,x.confidence);return {sum:a.sum+Number(x.score)*k,weight:a.weight+k};},{sum:0,weight:0});
...
export async function getMasteryOverview(userId:string):Promise<MasteryOverviewResponse>{
  ...
  const courses=[...ids].sort().map(courseId=>{
    const skills=summarize(snapshots.filter(x=>x.courseId===courseId)); const a=aggregate(skills);
