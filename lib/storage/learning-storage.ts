...
export const LEARNING_KEY = "deutschimo-learning-v11";
export const LEGACY_LEARNING_KEY = "deutschimo-learning-v8";
export const ACTIVE_USER_KEY = "deutschimo-active-learning-user";
export const CONTENT_KEY = "deutschimo-content-v10";
export const LEARNING_EVENT = "deutschimo-learning-updated";

function activeLearningUserId() {
  if (typeof window === "undefined") return "guest";
  return window.localStorage.getItem(ACTIVE_USER_KEY) || "guest";
}

function scopedLearningKey() {
  return `${LEARNING_KEY}:${activeLearningUserId()}`;
}

export function setActiveLearningUser(userId: string | null) {
...
export function getActiveLearningUser() {
  return activeLearningUserId();
}
...
export function readLearningState(): LearningState {
  const fallback = structuredClone(emptyLearningState);
  const scoped = readJson<LearningState | null>(scopedLearningKey(), null);
  if (scoped) return scoped;
  if (typeof window !== "undefined") {
    const legacy = readJson<LearningState | null>(LEGACY_LEARNING_KEY, null);
    if (legacy && activeLearningUserId() !== "guest") {
      writeJson(scopedLearningKey(), legacy);
      return legacy;
    }
  }
  return fallback;
...
export function writeLearningState(state: LearningState) {
  writeJson(scopedLearningKey(), state);
}

export function resetLearningState() {
  const next = structuredClone(emptyLearningState);
  next.userId = activeLearningUserId();
  writeLearningState(next);
}

export function clearCurrentLearningCache() {
  if (typeof window === "undefined") return;
  window.localStorage.removeItem(scopedLearningKey());
  window.dispatchEvent(new Event(LEARNING_EVENT));
}
