...
// V46_ROUTE_GUARD_E2E_CONTRACT
// A protected route can be guarded directly in page.tsx OR through a shared
// layout/middleware/auth boundary. Static source inspection alone cannot prove
// runtime protection in every valid Next.js architecture, so V46 also requires
// an explicit Playwright unauthenticated-route assertion.
...
function hasE2ERouteGuard(route) {
  const doubleQuoted = `page.goto("/${route}")`;
  const singleQuoted = `page.goto('/${route}')`;
  const templateQuoted = "page.goto(`/" + route + "`)";
  const hasGoto =
    authE2E.includes(doubleQuoted) ||
    authE2E.includes(singleQuoted) ||
    authE2E.includes(templateQuoted);

  if (!hasGoto) return false;

  // V46.1 must explicitly assert redirect to /auth.
...
for (const [label, route, pageSource] of protectedPages) {
  if (!hasDirectGuard(pageSource) && !hasE2ERouteGuard(route)) {
    errors.push(
      `${label}: authenticated route guard contract kaniti bulunamadi (page guard veya V46.1 E2E assertion gerekli).`,
    );
  }
}
...
for (const token of [
  ...
  "onboardingCompleted",
  "PLACEMENT_REQUIRED",
  "dailyStudyPlan.deleteMany",
]) {
  if (!onboarding.includes(token)) errors.push(`onboarding contract eksik: ${token}`);
}

const progress = read("app/api/progress/route.ts");
if (!/export\s+(?:async\s+)?function\s+GET|export\s+const\s+GET/.test(progress)) {
  errors.push("progress route GET handler eksik.");
}
if (!/export\s+(?:async\s+)?function\s+(PUT|POST)|export\s+const\s+(PUT|POST)/.test(progress)) {
  errors.push("progress route yazma handler'i eksik.");
}
if (progress.includes("captureMasteryExchange")) {
  errors.push("V37 ilkesi ihlal edildi: completion/progress Mastery evidence sayilmamali.");
}
...
for (const rel of specs) requireFile(rel);
